...
export const update = async(req, res)=>{
    try {
        ...
        if(!title || !text || !banner){
            return res.status(400).json({message:"Submit all fields to update the post!"});
        }
        ...
        if(String(news.user._id) !== req.userId){
            return res.status(400).send({
                message: "You did not create this post",
            })
        }

        await updateService(id, title, text, banner);
        return res.send({message: "Post seccessfully updated!"});
...
export const newsDelete = async(req, res) =>{
    try {
        ...
        if(String(news.user._id) !== req.userId){
            return res.status(400).send({
                message: "You did not delete this news",
            });
        }
...
export const likeNew = async(req, res) =>{
    try {
        const {id} = req.params;
...
export const deleteComment = async(req, res) =>{
    try {
        ...
        if(!commentFinder){
            return res.status(400).send({message:"Comment not foundo!"})
        }
        if(commentFinder.userId !== userId){
            return res.status(400).send({message: "You cant delete this comment"})
        }
        res.send({
